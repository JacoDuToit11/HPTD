...
from DeBERTa import DebertaV3ForPreTraining, DebertaV2PreTrainedModel
import torch
...
class PromptModel(DebertaV2PreTrainedModel):
    def __init__(self, config, dlm_loss, generator_name, text_mask, template_at_end, class_bias):
        ...
        self.deberta = DebertaV3ForPreTraining.from_pretrained(self.model_name, config=config)
    ...
    def MyGraphEmbedding(self, level_num_labels, label_dict, path_list, graph_type, graph_layers):
        ...
        input_embeds = self.deberta.get_input_embeddings()
    ...
    def tokenize_hard(self, dataset, level_num_labels, depth2label, label_dict):
        ...
        # Then these would capture some hierarchical meaning as well? See HPTModel tokenize_soft!
        ...
        for i in range(len(label_word_ids_flat)):
            self.deberta.deberta.embeddings.word_embeddings.weight.data[label_word_ids_flat[i]] = label_embeddings[i]
            if self.dlm_loss:
    ...
    def tokenize_soft(self, dataset, level_num_labels, depth2label, label_dict, path_list, graph_type, graph_layers, add_prompts_after_hier_prompt):
        ...
        for i in range(len(prompt_ids)):
            self.deberta.deberta.embeddings.word_embeddings.weight.data[prompt_ids[i]] = prompt_embeddings[i]
            if self.dlm_loss:
    ...
    def forward(self, input_ids, attention_mask, position_ids, token_type_ids, label_positions, text_length, training_data):
        ...
        outputs = self.deberta(input_ids, attention_mask = attention_mask, position_ids = position_ids, token_type_ids = token_type_ids)
        raw_logits = outputs
        if self.dlm_loss and training_data:
                text_logits = []
                text_is_replaced = []
